                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="text-center">
                   ...
                   <div className="space-y-2 text-sm">
                    ...
                    {/* Factores Aplicados en la Valuación */}
                    <div className="pt-4 border-t">
                      <h4 className="text-sm font-medium mb-3">Factores Aplicados en el Avalúo</h4>
                      <div className="space-y-2 text-xs">
                        {/* Tipo de Acceso */}
                        {propertyData.tipoAcceso && (
                          <div className="flex justify-between">
                            <span>
                              {propertyData.tipoAcceso === 'mainStreet' && 'Calle Principal:'}
                              {propertyData.tipoAcceso === 'vehicularPassage' && 'Pasaje Vehicular:'}
                              {propertyData.tipoAcceso === 'pedestrianPassage' && 'Pasaje Peatonal:'}
                              {propertyData.tipoAcceso === 'rightOfWay' && 'Servidumbre de Paso:'}
                            </span>
                            <span className={`font-medium ${
                              propertyData.tipoAcceso === 'mainStreet' ? 'text-green-600' :
                              propertyData.tipoAcceso === 'vehicularPassage' ? 'text-gray-600' :
                              'text-red-600'
                            }`}>
                              {propertyData.tipoAcceso === 'mainStreet' && '+10%'}
                              {propertyData.tipoAcceso === 'vehicularPassage' && '0%'}
                              {propertyData.tipoAcceso === 'pedestrianPassage' && '-8%'}
                              {propertyData.tipoAcceso === 'rightOfWay' && '-12%'}
                            </span>
                          </div>
                        )}
                        ...
                        {propertyData.ubicacion && (
                          <div className="flex justify-between">
                            <span>Calidad de Ubicación:</span>
                            <span className={`font-medium ${
                              propertyData.ubicacion === 'excelente' ? 'text-green-600' :
                              propertyData.ubicacion === 'buena' ? 'text-blue-600' :
                              propertyData.ubicacion === 'regular' ? 'text-yellow-600' :
                              'text-red-600'
                            }`}>
                              {propertyData.ubicacion === 'excelente' && '+40%'}
                              {propertyData.ubicacion === 'buena' && '+20%'}
                              {propertyData.ubicacion === 'regular' && '0%'}
                              {propertyData.ubicacion === 'mala' && '-30%'}
                            </span>
                          </div>
                        )}
                        ...
                        {propertyData.estadoGeneral && (
                          <div className="flex justify-between">
                            ...
                            <span className={`font-medium ${
                              ['nuevo', 'bueno'].includes(propertyData.estadoGeneral) ? 'text-green-600' :
                              ['medio', 'regular'].includes(propertyData.estadoGeneral) ? 'text-yellow-600' :
                              'text-red-600'
                            }`}>
                              {propertyData.estadoGeneral === 'nuevo' && '0%'}
                              {propertyData.estadoGeneral === 'bueno' && '-0.32%'}
                              {propertyData.estadoGeneral === 'medio' && '-2.52%'}
                              {propertyData.estadoGeneral === 'regular' && '-8.09%'}
                              {propertyData.estadoGeneral === 'reparaciones-sencillas' && '-18.1%'}
                              {propertyData.estadoGeneral === 'reparaciones-medias' && '-33.2%'}
                              {propertyData.estadoGeneral === 'reparaciones-importantes' && '-52.6%'}
                              {propertyData.estadoGeneral === 'danos-graves' && '-75.2%'}
                              {propertyData.estadoGeneral === 'en-desecho' && '-86.5%'}
                            </span>
                          </div>
                        )}
                        ...
                        {propertyData.antiguedad > 0 && (
                          <div className="flex justify-between">
                            <span>Depreciación por Antigüedad:</span>
                            <span className="font-medium text-orange-600">
                              {(() => {
                                ...
                                const vidaUtil = vidasUtiles[propertyData.tipoPropiedad as keyof typeof vidasUtiles] || 80;
                                const depreciacion = vidaUtil > 0 ? (propertyData.antiguedad / vidaUtil) * 100 : 0;
                                return `-${Math.min(depreciacion, 100).toFixed(1)}%`;
                              })()}
                            </span>
                          </div>
                        )}
